...
class self_atten(nn.Module):
    def __init__(self, inplanes):
        ...
        self.chanel_in = inplanes

        self.query_conv = nn.Conv2d(inplanes, inplanes//8, kernel_size=1)
        self.key_conv = nn.Conv2d(inplanes, inplanes//8, kernel_size=1)
        self.value_conv = nn.Conv2d(inplanes, inplanes, kernel_size=1)

        self.u = nn.Parameter(torch.zeros(1))
        self.softmax = nn.Softmax(dim = -1)

    def forward(self, x):
        m_batchsize, num_chanel, width, height = x.size()
        ...
        out = out.view(m_batchsize, num_chanel, width, height)
...
class GEN_DEEP(nn.Module):
    def __init__(self, ngpu=1):
        ...
        res_units = [64, 32, 16]
        # inp_res_units = [
        #     [32, 32, 32, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256, 256,
        #      256], [256, 168, 168], [168, 32, 32]]
        # inp_res_units = [
        #     [32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32,
        #      32], [32, 16, 16], [16, 16, 16]]
        inp_res_units = [
        ...
        self.a1 = nn.Sequential(nn.Conv2d(64, 32, 1, 1))
        self.a2 = nn.Sequential(nn.Conv2d(32, 16, 1, 1))

        self.layers_in = conv3x3(1, 64)
        ...
        layers.append(nn.Conv2d(nunits, 1, kernel_size=1, stride=1))
